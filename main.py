...
if __name__=="__main__":
 while True:
   ...
   table.add_row(f'1:{psutil.cpu_percent(interval=0.1,percpu=True)[1]}%',f'{psutil.cpu_freq(percpu=True)[1][0]} MHz',f'AVAIL:{round(psutil.virtual_memory()[1]/1e+9,2)} GB',f'{f"\n".join([x for x in mapped])}')
   time.sleep(1)
